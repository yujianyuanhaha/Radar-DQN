function RadarMDPSim(SimMethod, TrajFormat, TargetTravelMode, NumRuns, NumEvaluations, EvalMethod, EvalTraj, EnableExport, IncludeTrajPlot, NumBands, NumStatesInMemory, InterferenceBehavior, varargin)
...
t = 0:TimeInterval:(TimeSteps)*TimeInterval;
...
policy = NumActions*ones(NumStates,1);
...
switch SimMethod
    ...
    case 'Uniform'
        NumTrainingRuns = NumRuns*size(TargetPositions, 1)*size(TargetVelocities, 1);
...
for kk = 1:(NumTrainingRuns)
    ...
    for i=0:length(t)-1  
        ...
        switch InterferenceBehavior
            ...
            case 'DIRECTION-DEPENDENT-INTER' % Direction-dependent intermittent interferer
                [NewInt] = UpdateInterference(InterferenceBehavior, OriginalIntf(:,:,1), IntermProb, State(1), states_with_interference, IntfMask);
        end
        CurrentInt(:,:,1) = NewInt;
        ...
        P_hist_sparse{CurrentActionNumber}(OldStateNumber,StateNumber) = P_hist_sparse{CurrentActionNumber}(OldStateNumber,StateNumber) + 1;

        CurrentReward = CalculateReward(SINRdB, CurrentAction, NumBands);
        RewardCount_sparse{CurrentActionNumber}(OldStateNumber, StateNumber) = RewardCount_sparse{CurrentActionNumber}(OldStateNumber, StateNumber) + 1;
        R_sparse_unnormalized{CurrentActionNumber}(OldStateNumber, StateNumber) = R_sparse_unnormalized{CurrentActionNumber}(OldStateNumber, StateNumber) + CurrentReward;
...
% in Chris [~,obj.policy] = mdp_policy_iteration(obj.avgStateTrans,obj.rewardTrans,obj.discountFactor);
...
switch InterferenceBehavior
    ...
    case 'INTER'
        foldername = sprintf('%s-%s-%s-P0%2G-%dMEMSTATE-Results', FolderDateTimeStr, 'INTER', sprintf('%d', OriginalIntf), IntermProb*100, NumStatesInMemory);
    case 'AVOID'
